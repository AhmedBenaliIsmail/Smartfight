...
class NotificationController extends AbstractController
{
    ...
    public function snapshot(NotificationRepository $repo): Response
    {
        $user = $this->getUser();
        if (!$user) {
            return $this->json(['unreadCount' => 0, 'latest' => null]);
        }

        $unread = $repo->findUnreadByUser($user->getUserId());
        $latest = !empty($unread) ? $unread[0] : null;

        return $this->json([
            'unreadCount' => count($unread),
            'latest' => $latest ? [
                'id' => $latest->getNotificationId(),
                'title' => $latest->getMessage(),
                'type' => $latest->getType(),
                'createdAt' => $latest->getCreatedAt()->format('c')
            ] : null
        ]);
    }
}
