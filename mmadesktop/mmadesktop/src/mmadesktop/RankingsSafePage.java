...
public class RankingsSafePage {
    private final RankingService rankingService = new RankingService();
    private final BorderPane root = new BorderPane();
    private final TableView<Fighter> table = new TableView<>();

    public RankingsSafePage() {
        Button refresh = new Button("Refresh");
        refresh.setOnAction(e -> loadData());
        HBox top = new HBox(refresh);

        TableColumn<Fighter, String> rank = new TableColumn<>("Rank");
        rank.setCellValueFactory(c -> new SimpleStringProperty(String.valueOf(table.getItems().indexOf(c.getValue()) + 1)));
        ...
        table.getColumns().addAll(rank, name, elo);
